        ...
        if (actionObj && actionObj.Type = 1) {
            ...
            if (IsObject(ActionDescList) && ActionDescList.Has(key)) {
                ...
                if (actionDesc.Length >= 2)
                    ...
                    current_keyMap .= HotKeyStr "`t" ActionDescList[key] "`n"
            } else {
                HotKeyStr := vim.CheckCapsLock(vim.Convert2VIM(key))
                HotKeyStr := vim.ShiftUpper(HotKeyStr)
                current_keyMap .= HotKeyStr "`t" ActionDescList "`n"
            }
        } else if (actionObj) {
            ; 修复: 使用正确的变量名
            ShiftKey := key
            ...
            HotKeyStr := vim.CheckCapsLock(vim.Convert2VIM(key))
            HotKeyStr := vim.ShiftUpper(HotKeyStr)
            current_keyMap .= HotKeyStr "`t" actionObj.Comment "`n"
        }
    }
