...
### 配置分工
- **主配置文件** (`config/vimd.ini`): 管理插件的启用状态和默认模式
  - `[plugins]`: 控制插件是否启用
...
### AfterEffects插件
- **配置文件**: `plugins\AfterEffects\AfterEffects.ini`
- **配置内容**:
```ini
[Config]
EnableLogging=false
...
set_time_out=300
ime_enabled=true
ime_enable_debug=false
ime_check_interval=200
ime_enable_mouse_click=true
ime_max_retries=3
ime_auto_switch_timeout=5000
```

### Blender插件
- **配置文件**: `plugins\Blender\Blender.ini`
- **配置内容**:
```ini
[Blender]
python_path=C:\Program Files\Blender Foundation\Blender 4.5\4.5\python\bin\python.exe
set_time_out=300
```

### Max3D插件
- **配置文件**: `plugins\Max3D\Max3D.ini`
- **配置内容**:
```ini
[Max3D]
set_time_out=300
...
ime_max_retries=3
ime_auto_switch_timeout=5000
ime_special_handling=ignoreIBeamWithoutControl
```
