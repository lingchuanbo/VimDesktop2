...
// 20251128
{
    ...
    function collectSelectedFootages() {
        ...
        for (var k = 0; k < itemsToProcess.length; k++) {
            ...
            // 判断是否为序列素材 - 以项目中的导入方式为准
            var isSequence = false;

            // 方法1: 检查 mainSource.isSequence 属性（最准确的方法）
            if (item.mainSource && item.mainSource.isSequence === true) {
                isSequence = true;
            }

            // 方法2: 如果方法1不可靠，检查 duration 和 frameDuration
            // 序列图的 duration 会大于单帧的 frameDuration
            if (!isSequence && item.duration > item.frameDuration) {
                // 排除视频文件
            ...
            if (isSequence) {
                ...
                // 根据文件扩展名创建对应的文件夹（png→png文件夹，tga→tga文件夹等）
