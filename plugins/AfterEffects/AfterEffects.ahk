...
AfterEffects_RenderFilesWithTC() {
    ...
    setPreset := PluginCatalog.GetResourcePath("AfterEffects", "Script\\OpenLocalFilesRenderTCAtive.jsx")
...
GetAfterEffectsScriptPaths() {
    ; 基础目录
    baseDir := PluginCatalog.GetResourcePath("AfterEffects", "Script")
...
AfterEffects_HandleExpression(filePath) {
    try {
        ...
        ; 清除文件结尾空字符
        expressionContent := Trim(expressionContent, "`r`n ")

        ; 移除回车符，确保统一行结束
        expressionContent := StrReplace(expressionContent, "`r", "")
...
AfterEffects_ApplyExpressionScript(expressionContent, fileName) {
    try {
        ...
        ; 创建临时JSX脚本
        tempScriptPath := PluginCatalog.GetResourcePath("AfterEffects", "Script\\temp_apply_expression.jsx")
        ; JSX脚本内容
        jsxCode := "try {`n"
